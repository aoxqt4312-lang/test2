...
public class MainActivity extends Activity {
	...
	"Кстати, это приложение стирает FRP (если вы не отключали это в дополнительных параметрах). Всё что написано далее, написано как дополнительная предострожность, например на случай если это был авто-сброс или если система не позволила стиреть FRP.\n\n"+
			
	"Избегайте FRP (Factory Reset Protection). Не храните аккаунты Google на телефоне в основном профиле. Их ID остаются после сброса данных, так как сброс данных без ввода пароля не удаляет FRP. Данные уже удалены, но ID аккаунта Google остаётся. Зная ваш ID, можно узнать вашу почту. Зная вашу почту, можно узнать ваш никнейм и так далее. А если к аккаунту Google вы решили привязать бекап, то это ещё опаснее. В общем, не делайте бекапы или не привязывайте их к аккаунту Google, а чтобы избежать самого FRP, создайте рабочий профиль через Shelter или лучше ProtectedWorkProfile (они есть на F-droid) и перенесите туда аккаунты Google — ведь оттуда аккаунты не сохраняются в FRP. Перед сбросом у вас не должно быть Google-аккаунтов в основном профиле.";
    ...
    "Tips:\n" +
    "Make sure there are no other keyboards you can switch to on the lock screen. If there are, disable them so you can't be pressured to switch to them. How?\n" +
	...
	"By the way, this application wipes the FRP (if you didn't disable it in additional options). Everything that is written next is written as an extra precaution, for example, for instance if it was auto-wipe or if the system didn't allow to wipe the FRP.\n\n" +
			
	"Avoid FRP (Factory Reset Protection). Don't store Google accounts on your phone in the main profile. Their IDs remain after data wipe, because data wipe without entering the password does not remove FRP. Data is already wiped, but the Google account ID remains. Knowing your account ID, they can find your email. Knowing your email, they can find your username, and so on. And if you decided to link backups to the Google account, that's even more dangerous. In general, don't make backups or don't link them to the Google account, and to avoid FRP itself, create a work profile through Shelter or better ProtectedWorkProfile (they are on F-droid) and move Google accounts there—from there the accounts are not retained in FRP. Before resetting, you must not have any Google accounts in the main profile.";
